...
public class TeacherSessionController {
    ...
    @PostMapping("/save")
    public String save(@ModelAttribute TeacherSession teacherSession, RedirectAttributes redirectAttributes) {

        boolean isNew = teacherSession.getId() == null;
        teacherSessionService.save(teacherSession);

        redirectAttributes.addFlashAttribute("success",
                isNew ? "Teacher session added successfully." : "Teacher session updated successfully.");

        if (isNew) {
            activityLogService.logCreate("Teacher Session", "Added teacher session #" + teacherSession.getId());
        } else {
            activityLogService.logUpdate("Teacher Session", "Updated teacher session #" + teacherSession.getId());
        }
    ...
    public String delete(@PathVariable Long id, RedirectAttributes redirectAttributes) {
        ...
        redirectAttributes.addFlashAttribute("success", "Teacher session deleted successfully.");

        activityLogService.logDelete("Teacher Session", "Deleted teacher session #" + id);
    ...
    public String reassignSave(
        ...
        if (teacherSessionIds != null && !teacherSessionIds.isEmpty()) {
            ...
            redirectAttributes.addFlashAttribute("success",
                    teacherSessionIds.size() + " assignment(s) reassigned successfully.");

            activityLogService.logUpdate("Teacher Session", "Reassigned " + teacherSessionIds.size()
                    + " assignment(s) to session #" + toSessionId);
