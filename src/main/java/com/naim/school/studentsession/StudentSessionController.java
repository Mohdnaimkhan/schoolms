...
public class StudentSessionController {
    ...
    @PostMapping("/save")
    public String save(@ModelAttribute StudentSession studentSession, RedirectAttributes redirectAttributes) {

        boolean isNew = studentSession.getId() == null;
        studentSessionService.save(studentSession);

        redirectAttributes.addFlashAttribute("success",
                isNew ? "Student session added successfully." : "Student session updated successfully.");

        if (isNew) {
            activityLogService.logCreate("Student Session", "Added student session #" + studentSession.getId());
        } else {
            activityLogService.logUpdate("Student Session", "Updated student session #" + studentSession.getId());
        }
    ...
    public String delete(@PathVariable Long id, RedirectAttributes redirectAttributes) {
        ...
        redirectAttributes.addFlashAttribute("success", "Student session deleted successfully.");

        activityLogService.logDelete("Student Session", "Deleted student session #" + id);
    ...
    public String promoteSave(
        ...
        if (studentSessionIds != null && !studentSessionIds.isEmpty()) {
            ...
            redirectAttributes.addFlashAttribute("success",
                    studentSessionIds.size() + " student(s) promoted successfully.");

            activityLogService.logUpdate("Student Session", "Promoted " + studentSessionIds.size()
                    + " student(s) to session #" + toSessionId);
